...
const uint8_t SET_ADV_DATA[] =   //Adv data format. Set according to user select profile. See Bluetooth Spec. Ver5.0 [Vol 3], Part C, Section 11
{
    ...
    GAP_AD_TYPE_LENGTH_3, GAP_AD_TYPE_SERVICE_MORE_16B_UUID, GATT_SPEC_SERVICES_BATTERY_SERVICE, GATT_SPEC_SERVICES,
...
extern Uint8 ATT_HDL_BAS_PRIMARY_SERVICE[];
extern Uint8 ATT_HDL_BAS_CHARACTERISTIC_HEART_RATE_MEASUREMENT[];
extern Uint8 ATT_HDL_BAS_BATTERY_LEVEL_INIT[];
extern Uint8 ATT_HDL_BAS_CLIENT_CHARACTERISTIC_CONFIGURATION_INIT[];
extern Uint8 att_HDL_BAS_BATTERY_LEVEL[];
extern Uint8 att_HDL_BAS_CLIENT_CHARACTERISTIC_CONFIGURATION[];
extern Uint8 att_HDL_CSCS_CYCLING_POWER_FEATURE[];
extern const Uint8 ATT_HDL_CSCS_CYCLING_POWER_FEATURE[];
extern const Uint8 ATT_HDL_CSCS_CHARACTERISTIC_CYCLING_POWER_FEATURE[];
extern Uint8 att_HDL_CSCS_CLIENT_CHARACTERISTIC_CONFIGURATION[];
extern Uint8 att_HDL_CSCS_HEART_RATE_MEASUREMENT[];
extern const Uint8 ATT_HDL_CSCS_CLIENT_CHARACTERISTIC_CONFIGURATION_INIT[];
extern const Uint8 ATT_HDL_CSCS_HEART_RATE_MEASUREMENT_INIT[];
extern const Uint8 ATT_HDL_CSCS_CHARACTERISTIC_HEART_RATE_MEASUREMENT[];
...
void BleApp_Main(void)
{
    ...
    else if (ble_state == STATE_BLE_CONNECTION)
    {
        ...
        //Hearte Rate type
        if ((att_HDL_CSCS_HEART_RATE_MEASUREMENT[0]&BIT1) == 0) //initial is 0x14 & 0x02 = 0, toggle "device detected" / "device not detected" information
        {
            att_HDL_CSCS_HEART_RATE_MEASUREMENT[0] |= BIT1;  //set Sensor Contact Status bit
        }
        else
        {
            att_HDL_CSCS_HEART_RATE_MEASUREMENT[0] &= ~BIT1;  //clear Sensor Contact Status bit
        }

        //If (att_HDL_HRS_CLIENT_CHARACTERISTIC_CONFIGURATION - Notify), then send out data
        if (ATT_HDL_Notify(connId, (uint8_t *)ATT_HDL_CSCS_HEART_RATE_MEASUREMENT_INIT, att_HDL_CSCS_CLIENT_CHARACTERISTIC_CONFIGURATION, att_HDL_CSCS_HEART_RATE_MEASUREMENT, 0x0b) == BLESTACK_STATUS_SUCCESS)
        {
            att_HDL_CSCS_HEART_RATE_MEASUREMENT[1]++;             //+1, Heart Rate Data. Here just a simulation, increase 1 about every second
            att_HDL_CSCS_HEART_RATE_MEASUREMENT[2]++;             //+1, Heart Rate RR-Interval
        }

        if (ATT_HDL_Notify(connId, (uint8_t *)ATT_HDL_BAS_BATTERY_LEVEL_INIT, att_HDL_BAS_CLIENT_CHARACTERISTIC_CONFIGURATION, att_HDL_BAS_BATTERY_LEVEL, 4) == BLESTACK_STATUS_SUCCESS)
        {
            att_HDL_BAS_BATTERY_LEVEL[1]++;             //+1, Heart Rate Data. Here just a simulation, increase 1 about every second
            att_HDL_BAS_BATTERY_LEVEL[2]++;             //+1, Heart Rate RR-Interval
        }
